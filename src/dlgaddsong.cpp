...
DlgAddSong::DlgAddSong(TableModelRotation &rotationModel,
                       TableModelQueueSongs &queueModel,
                       int songId,
                       QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::DlgAddSong)
    ...
    , m_songId(songId)
{
    ...
    connect(ui->spinBoxKeyChange,
            qOverload<int>(&QSpinBox::valueChanged),
            this,
            &DlgAddSong::spinBoxKeyChangeChanged);
    connect(ui->comboBoxRotSingers,
            qOverload<int>(&QComboBox::activated),
            this,
            &DlgAddSong::comboBoxRotSingersActivated);
    connect(ui->lineEditNewSingerName,
            &QLineEdit::returnPressed,
            this,
            &DlgAddSong::pushButtonAddClicked);
    connect(ui->lineEditNewSingerName,
            &QLineEdit::textChanged,
            this,
            &DlgAddSong::lineEditNewSingerNameTextChanged);
    connect(ui->comboBoxRegSingers,
            qOverload<int>(&QComboBox::activated),
            this,
            &DlgAddSong::comboBoxRegSingersActivated);
}
...
void DlgAddSong::pushButtonAddClicked()
{
    ...
    case 0:
        if (ui->lineEditNewSingerName->text() == QString()) {
            QMessageBox::warning(this,
                                 "No singer name specified",
                                 "You must enter a new singer name to add the song to.");
        } else if (m_rotModel.singerExists(ui->lineEditNewSingerName->text())) {
            QMessageBox msgBox;
            msgBox.setText("A singer with that name already exists");
            msgBox.setInformativeText(
                "Would you like to add the song to the existing singer instead?");
            ...
            int singerId = m_rotModel.singerAdd(ui->lineEditNewSingerName->text(),
                                                ui->comboBoxPosition->currentIndex());
            m_queueModel.songAddSlot(m_songId, singerId, ui->spinBoxKeyChange->value());
            emit newSingerAdded(m_rotModel.getSinger(singerId).position);
            close();
        }
    ...
    case 2:
        if (m_rotModel.singerExists(ui->comboBoxRegSingers->currentText())) {
            ...
            if (m_rotModel.getSinger(existingId).regular) {
                ...
                auto answer
                    = QMessageBox::question(this,
                                            ...
                                            "A singer with this name is currently in the rotation, "
                                            "but they aren't marked as a regular.\n"
                                            "Would you like to add the song to their queue and "
                                            "load this regular's song history for "
                                            "the matching rotation singer?",
                                            QMessageBox::StandardButtons(QMessageBox::Yes
                                                                         | QMessageBox::Cancel),
                                            QMessageBox::Yes);
                if (answer == QMessageBox::Yes) {
                    int singerId = m_rotModel.getSingerByName(ui->comboBoxRegSingers->currentText())
                                       .id;
            ...
            int singerId = m_rotModel.singerAdd(ui->comboBoxRegSingers->currentText(),
                                                ui->comboBoxPosition->currentIndex());
            m_rotModel.singerMakeRegular(singerId);
            m_queueModel.songAddSlot(m_songId, singerId, ui->spinBoxKeyChange->value());
            emit newSingerAdded(m_rotModel.getSinger(singerId).position);
