...
DlgRequests::DlgRequests(TableModelRotation &rotationModel,
    ...
    , ui(new Ui::DlgRequests)
{
    ...
    dir.mkpath(logDir);
    QString logFilePath;
    QString filename = "openkj-requests-" + QDateTime::currentDateTime().toString("yyyy-MM-dd")
                       + ".log";
    ...
    ui->tableViewRequests->viewport()->installEventFilter(
        new TableViewToolTipFilter(ui->tableViewRequests));
    ...
    ui->tableViewSearch->viewport()->installEventFilter(
        new TableViewToolTipFilter(ui->tableViewSearch));
    ui->groupBoxAddSong->setDisabled(true);
    connect(ui->tableViewRequests->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
            &DlgRequests::requestSelectionChanged);
    connect(ui->tableViewSearch->selectionModel(),
            &QItemSelectionModel::selectionChanged,
            this,
    ...
    connect(ui->comboBoxAddPosition,
    ...
    connect(ui->lineEditSearch,
            &CustomLineEdit::escapePressed,
            this,
            &DlgRequests::lineEditSearchEscapePressed);
    connect(ui->checkBoxDelOnAdd,
            &QCheckBox::clicked,
            &m_settings,
            &Settings::setRequestRemoveOnRotAdd);
    ui->cbxAutoShowRequestsDlg->setChecked(m_settings.requestDialogAutoShow());
    connect(ui->cbxAutoShowRequestsDlg,
            &QCheckBox::clicked,
            &m_settings,
            &Settings::setRequestDialogAutoShow);
...
{
    QString thm = (m_settings.theme() == 1) ? ":/theme/Icons/okjbreeze-dark/"
                                            : ":/theme/Icons/okjbreeze/";
...
void DlgRequests::requestSelectionChanged(const QItemSelection &current,
                                          const QItemSelection &previous)
{
    ...
    Q_UNUSED(previous);
    if ((index.isValid())
        && (ui->tableViewRequests->selectionModel()->selectedIndexes().size() > 0)) {
        ...
        QString filterStr = index.sibling(index.row(), 1).data().toString() + " "
...
void DlgRequests::on_pushButtonClearReqs_clicked()
{
    ...
    msgBox.setInformativeText(
        "This action will clear all received requests. This operation can not be undone.");
...
void DlgRequests::on_tableViewRequests_clicked(const QModelIndex &index)
{
    if (index.column() == 5) {
        ...
        m_reqLogger->info("RequestID: {} | Manually removed by host",
...
void DlgRequests::on_pushButtonAddSong_clicked()
{
    ...
    if (!pastedUrl.isEmpty()) {
        ...
        QModelIndex reqIndex = hasRequestSelected
                                   ? ui->tableViewRequests->selectionModel()->selectedIndexes().at(0)
                                   : QModelIndex();
        // Columns on tableViewRequests: 0=Singer, 1=Artist, 2=Title, matching
        // requestSelectionChanged()'s already-verified mapping.
        QString pastedArtist = reqIndex.isValid()
                                   ? reqIndex.sibling(reqIndex.row(), 1).data().toString()
                                   : QString();
        QString pastedTitle = reqIndex.isValid()
                                  ? reqIndex.sibling(reqIndex.row(), 2).data().toString()
                                  : QString();

        if (ui->radioButtonNewSinger->isChecked()) {
            if (ui->lineEditSingerName->text() == ""
                || rotModel.singerExists(ui->lineEditSingerName->text()))
                return;
            ...
            m_reqLogger->info("RequestID: {} | Added pasted link to new singer | Name: {} | "
                              "Position: {} | Wait: {} | URL: {}",
                              logRequestId,
            ...
            emit addRequestPastedLink(pastedUrl,
                                      pastedArtist,
                                      pastedTitle,
                                      rotModel.getSingerByName(ui->comboBoxSingers->currentText())
                                          .id);
            m_reqLogger
                ->info("RequestID: {} | Added pasted link to existing singer | Name: {} | URL: {}",
                       logRequestId,
        ...
        if (hasRequestSelected && m_settings.requestRemoveOnRotAdd()) {
            ...
            m_reqLogger->info("RequestID: {} | Auto-removed after add to singer queue",
                              curRequestId);
            m_reqLogger->flush();
            ui->tableViewRequests->selectionModel()->clearSelection();
            ui->lineEditSearch->clear();
    ...
    if (song->isStream) {
        ...
        if (ui->radioButtonNewSinger->isChecked()) {
            ...
            m_reqLogger->info("RequestID: {} | Added stream to new singer | Name: {} | Position: "
                              "{} | Wait: {} | Song: {} - {}",
                              logRequestId,
                              ui->lineEditSingerName->text().toStdString(),
                              rotModel.getSinger(newSingerId).position,
            ...
            emit addRequestStreamSong(song->streamLibraryId,
                                      rotModel.getSingerByName(ui->comboBoxSingers->currentText())
                                          .id);
            m_reqLogger
                ->info("RequestID: {} | Added stream to existing singer | Name: {} | Song: {} - {}",
                       logRequestId,
                       ui->comboBoxSingers->currentText().toStdString(),
                       song->artist.toStdString(),
                       song->title.toStdString());
        ...
        m_reqLogger->info("RequestID: {} | Added to new singer | Name: {} | Position: {} | Wait: "
                          "{} | Song: {} - {} - {} | Key: {}",
                          logRequestId,
                          ui->lineEditSingerName->text().toStdString(),
                          rotModel.getSinger(newSingerId).position,
        ...
        emit addRequestSong(song->id,
                            rotModel.getSingerByName(ui->comboBoxSingers->currentText()).id,
                            keyChg);
        m_reqLogger->info(
            "RequestID: {} | Added to existing singer | Name: {} | Song: {} - {} - {} | Key: {}",
            logRequestId,
            ui->comboBoxSingers->currentText().toStdString(),
...
void DlgRequests::sslError()
{
    QMessageBox::warning(
        this,
        tr("SSL Handshake Error"),
        tr("An error was encountered while establishing a secure connection to the requests "
           "server.  This is usually caused by an invalid or self-signed cert on the server.  You "
           "can set the requests client to ignore SSL errors in the network settings dialog."));
}

void DlgRequests::delayError(int seconds)
{
    QMessageBox::warning(this,
                         tr("Possible Connectivity Issue"),
                         tr("It has been ") + QString::number(seconds)
                             + tr(" seconds since we last received a response from the requests "
                                  "server.  You may be missing new submitted requests.  Please "
                                  "ensure that your network connection is up and working."));
}
...
void DlgRequests::on_pushButtonUpdateDb_clicked()
{
    ...
    QMessageBox msgBox;
    msgBox.setText(tr("Are you sure?\n\nThis operation can take serveral minutes depending on the "
                      "size of your song database and the speed of your internet connection.\n"));
    ...
    if (ret == QMessageBox::Yes) {
        ...
        QApplication::processEvents();
        connect(&songbookApi,
                &OKJSongbookAPI::remoteSongDbUpdateNumDocs,
                progressDialog,
                &QProgressDialog::setMaximum);
        connect(&songbookApi,
                &OKJSongbookAPI::remoteSongDbUpdateProgress,
                progressDialog,
                &QProgressDialog::setValue);
        connect(progressDialog,
                &QProgressDialog::canceled,
                &songbookApi,
                &OKJSongbookAPI::dbUpdateCanceled);
        //    progressDialog->show();
...
void DlgRequests::lineEditSearchEscapePressed()
{
    ...
    QString filterStr = index.sibling(index.row(), 2).data().toString() + " "
...
void DlgRequests::autoSizeViews()
{
    ...
    ui->tableViewSearch->horizontalHeader()
        ->setSectionResizeMode(TableModelKaraokeSongs::COL_LASTPLAY, QHeaderView::ResizeToContents);
    ui->tableViewSearch->horizontalHeader()->setSectionResizeMode(TableModelKaraokeSongs::COL_PLAYS,
                                                                  QHeaderView::ResizeToContents);
    ui->tableViewSearch->horizontalHeader()
        ->setSectionResizeMode(TableModelKaraokeSongs::COL_DURATION, QHeaderView::ResizeToContents);
    ui->tableViewSearch->horizontalHeader()->setSectionResizeMode(TableModelKaraokeSongs::COL_ARTIST,
                                                                  QHeaderView::Stretch);
    ui->tableViewSearch->horizontalHeader()->setSectionResizeMode(TableModelKaraokeSongs::COL_TITLE,
                                                                  QHeaderView::Stretch);
    ui->tableViewSearch->horizontalHeader()->setSectionResizeMode(TableModelKaraokeSongs::COL_SONGID,
                                                                  QHeaderView::ResizeToContents);
    QApplication::processEvents();
    ui->tableViewSearch->horizontalHeader()->setSectionResizeMode(TableModelKaraokeSongs::COL_ARTIST,
    ...
    ui->tableViewSearch->horizontalHeader()->setSectionResizeMode(TableModelKaraokeSongs::COL_SONGID,
                                                                  QHeaderView::Interactive);
    ui->tableViewSearch->horizontalHeader()
        ->setSectionResizeMode(TableModelKaraokeSongs::COL_LASTPLAY, QHeaderView::Interactive);
    ui->tableViewSearch->horizontalHeader()->setSectionResizeMode(TableModelKaraokeSongs::COL_PLAYS,
                                                                  QHeaderView::Interactive);
    ui->tableViewSearch->horizontalHeader()
        ->setSectionResizeMode(TableModelKaraokeSongs::COL_DURATION, QHeaderView::Interactive);
    ...
    int singerColSize = QFontMetrics(m_settings.applicationFont())
                            .horizontalAdvance("_Isaac_Lightburn_");
#else
    ...
    int remainingSpace = ui->tableViewRequests->width() - tsWidth - delwidth - singerColSize
                         - keyWidth - 10;
...
void DlgRequests::on_pushButtonWebSearch_clicked()
{
    ...
    QString link = "https://www.youtube.com/results?search_query=%22karaoke%22+"
                   + encodedWords.join('+');
...
void DlgRequests::requestsChanged(OkjsRequests requests)
{
    ...
    for (OkjsRequest &request : requests) {
        ...
        auto result = std::find(m_prevRequestList.begin(),
                                m_prevRequestList.end(),
                                request.requestId);
        if (result == m_prevRequestList.end()) {
            m_reqLogger->info("RequestID: {} | Received new request |  Singer: {} | Submitted: {} "
                              "| Song: {} - {}",
                              request.requestId,
                              request.singer.toStdString(),
                              QDateTime::fromSecsSinceEpoch(request.time)
...
void DlgRequests::on_pushButtonDownload_clicked()
{
    ...
    if (!url.isEmpty()) {
        ...
        QModelIndex reqIndex = hasRequestSelected
                                   ? ui->tableViewRequests->selectionModel()->selectedIndexes().at(0)
                                   : QModelIndex();
        artist = reqIndex.isValid() ? reqIndex.sibling(reqIndex.row(), 1).data().toString()
                                    : QString();
        title = reqIndex.isValid() ? reqIndex.sibling(reqIndex.row(), 2).data().toString()
                                   : QString();
