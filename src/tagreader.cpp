...
void TagReader::setMedia(const QString &path)
{
    ...
    if ((path.endsWith(".mp3", Qt::CaseInsensitive)) || (path.endsWith(".ogg", Qt::CaseInsensitive))
        ...
        || path.endsWith(".m4v", Qt::CaseInsensitive)) {
...
#endif
    GError *err = nullptr;
    GstDiscovererInfo *discovererInfo = gst_discoverer_discover_uri(
        discoverer, uri.toUtf8().toPercentEncoding("!$&'()*+,;=:/?[]@").constData(), &err);
    if (err) {
...
void TagReader::taglibTags(const QString &path)
{
    ...
    if (!f.isNull()) {
        ...
        m_logger->info(
            "{} Taglib result: Artist: {} - Title: {} - Album: {} - Track: {} - Duration: {}",
            m_loggingPrefix,
            m_artist,
            m_title,
            m_album,
