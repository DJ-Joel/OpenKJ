...
{
    connect(ui->doubleSpinBoxLeft,
            qOverload<double>(&QDoubleSpinBox::valueChanged),
            &m_settings,
            &Settings::setBookCreatorMarginLft);
    connect(ui->doubleSpinBoxRight,
            qOverload<double>(&QDoubleSpinBox::valueChanged),
            &m_settings,
            &Settings::setBookCreatorMarginRt);
    connect(ui->doubleSpinBoxTop,
            qOverload<double>(&QDoubleSpinBox::valueChanged),
            &m_settings,
            &Settings::setBookCreatorMarginTop);
    connect(ui->doubleSpinBoxBottom,
            qOverload<double>(&QDoubleSpinBox::valueChanged),
            &m_settings,
            &Settings::setBookCreatorMarginBtm);
    connect(ui->fontCbxFooter,
            &QFontComboBox::currentFontChanged,
            this,
            &DlgBookCreator::saveFontSettings);
    connect(ui->spinBoxSizeFooter,
            qOverload<int>(&QSpinBox::valueChanged),
            this,
            &DlgBookCreator::saveFontSettings);
    connect(ui->checkBoxBoldFooter,
            qOverload<bool>(&QCheckBox::clicked),
            this,
            &DlgBookCreator::saveFontSettings);
    connect(ui->lineEditFooterText,
            &QLineEdit::textChanged,
            &m_settings,
            &Settings::setBookCreatorFooterText);
    connect(ui->fontCbxHeader,
            &QFontComboBox::currentFontChanged,
            this,
            &DlgBookCreator::saveFontSettings);
    connect(ui->spinBoxSizeHeader,
            qOverload<int>(&QSpinBox::valueChanged),
            this,
            &DlgBookCreator::saveFontSettings);
    connect(ui->checkBoxBoldHeader,
            qOverload<bool>(&QCheckBox::clicked),
            this,
            &DlgBookCreator::saveFontSettings);
    connect(ui->lineEditHeaderText,
            &QLineEdit::textChanged,
            &m_settings,
            &Settings::setBookCreatorHeaderText);
    connect(ui->fontCbxArtist,
            &QFontComboBox::currentFontChanged,
            this,
            &DlgBookCreator::saveFontSettings);
    connect(ui->spinBoxSizeArtist,
            qOverload<int>(&QSpinBox::valueChanged),
            this,
            &DlgBookCreator::saveFontSettings);
    connect(ui->checkBoxBoldArtist,
            qOverload<bool>(&QCheckBox::clicked),
            this,
            &DlgBookCreator::saveFontSettings);
    connect(ui->fontCbxTitle,
            &QFontComboBox::currentFontChanged,
            this,
            &DlgBookCreator::saveFontSettings);
    connect(ui->spinBoxSizeTitle,
            qOverload<int>(&QSpinBox::valueChanged),
            this,
            &DlgBookCreator::saveFontSettings);
    connect(ui->checkBoxBoldTitle,
            qOverload<bool>(&QCheckBox::clicked),
            this,
            &DlgBookCreator::saveFontSettings);
    connect(ui->doubleSpinBoxBottom,
    ...
    connect(ui->cbxColumns,
            qOverload<int>(&QComboBox::currentIndexChanged),
            &m_settings,
            &Settings::setBookCreatorCols);
    connect(ui->cbxPageSize,
            qOverload<int>(&QComboBox::currentIndexChanged),
            &m_settings,
...
QStringList DlgBookCreator::getArtists()
{
    QSqlQuery query;
    QStringList artists;
    QString sql = "SELECT DISTINCT artist FROM dbsongs WHERE discid != '!!BAD!!' AND discid != "
                  "'!!DROPPED!!' ORDER BY artist";
    query.exec(sql);
    while (query.next()) {
...
QStringList DlgBookCreator::getTitles(const QString &artist)
{
    QSqlQuery query;
    QStringList titles;
    QString sql = "SELECT DISTINCT title FROM dbsongs WHERE artist = :artist AND discid != "
                  "'!!BAD!!' AND discid != '!!DROPPED!!' ORDER BY title";
    query.prepare(sql);
...
void DlgBookCreator::writePdf(const QString &filename, int nCols)
{
    ...
    pdf.setPageSize(
        QPageSize(static_cast<QPageSize::PageSizeId>(ui->cbxPageSize->currentData().toInt())));
    pdf.setPageMargins(QMarginsF(ui->doubleSpinBoxLeft->value(),
                                 ui->doubleSpinBoxTop->value(),
                                 ui->doubleSpinBoxRight->value(),
                                 ui->doubleSpinBoxBottom->value()),
                       QPageLayout::Inch);
    ...
    while (!entries.isEmpty()) {
        ...
        if (ui->lineEditFooterText->text() != "" || m_settings.bookCreatorPageNumbering()) {
            ...
            if (m_settings.bookCreatorFooterText() != "")
                painter.drawText(0,
                                 painter.viewport().height() - fFontHeight,
                                 painter.viewport().width(),
                                 painter.fontMetrics().height(),
                                 Qt::AlignCenter,
                                 m_settings.bookCreatorFooterText());
            if (m_settings.bookCreatorPageNumbering()) {
                ...
                painter.drawText(painter.viewport().width() - txtRect.width() - 20,
                                 painter.viewport().height() - txtRect.height(),
                                 txtRect.width(),
                                 txtRect.height(),
                                 Qt::AlignRight,
                                 pageStr);
        ...
        painter.drawLine(painter.viewport().width(),
                         headerOffset,
                         painter.viewport().width(),
        ...
        while ((curDrawPos + fontHeight) <= (painter.viewport().height() - bottomOffset)) {
            ...
            if (entry.at(0) == QString("-")) {
                ...
                txtRect = painter.fontMetrics().boundingRect(entry);
                entry.remove(0, 1);
                painter.drawText(lineOffset + 200,
                                 curDrawPos,
                                 txtRect.width(),
                                 txtRect.height(),
                                 Qt::AlignLeft,
                ...
                txtRect = painter.fontMetrics().boundingRect(entry);
                entry.remove(0, 1);
                painter.drawText(lineOffset + 400,
                                 curDrawPos,
                                 txtRect.width(),
                                 txtRect.height(),
                                 Qt::AlignLeft,
        ...
        if (nCols == 3) {
            ...
            while ((curDrawPos + fontHeight) <= (painter.viewport().height() - bottomOffset)) {
                ...
                if (entry.at(0) == QString("-")) {
                    ...
                    txtRect = painter.fontMetrics().boundingRect(entry);
                    entry.remove(0, 1);
                    painter.drawText(lineOffset2 + 200,
                                     curDrawPos,
                                     txtRect.width(),
                                     txtRect.height(),
                                     Qt::AlignLeft,
                    ...
                    txtRect = painter.fontMetrics().boundingRect(entry);
                    entry.remove(0, 1);
                    painter.drawText(lineOffset2 + 400,
                                     curDrawPos,
                                     txtRect.width(),
                                     txtRect.height(),
                                     Qt::AlignLeft,
