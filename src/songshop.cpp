...
SongShop::SongShop(QObject *parent)
    : QObject(parent)
{
...
void SongShop::knLogin(QString userName, QString password)
{
    knLoginError = false;
    QByteArray md5hash
        = QCryptographicHash::hash(QByteArray::fromRawData((const char *) password.toLocal8Bit(),
                                                           password.length()),
                                   QCryptographicHash::Md5)
              .toHex();
    QString passHash = QString(md5hash);
    QString urlstr = "https://www.partytyme.net/songshop/cat/"
                     "api_account_setup.php?action=validate_login&username="
                     + userName + "&md5=" + passHash + "&merchant=99";
...
{
    reply->abort();
    m_logger->warn("{} Received SSL error when connecting to db.openkj.org, please make sure your "
...
void SongShop::onNetworkReply(QNetworkReply *reply)
{
    ...
    if (error) {
        m_logger->warn("{} Received error reply from server: {}",
                       m_loggingPrefix,
    ...
    if (command == "getsongs") {
        ...
        m_logger->info("{} Download complete", m_loggingPrefix);
               ...
               && (json.object().value("error").toString()
                   == "Payment failed. Check your credit card details.")) {
