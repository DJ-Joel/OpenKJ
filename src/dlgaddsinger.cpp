...
void DlgAddSinger::addSinger()
{
    if (m_rotModel.singerExists(ui->lineEditName->text())) {
        QMessageBox::warning(this,
                             "Unable to add singer",
                             "A singer with the same name already exists.");
        return;
    }
    ...
    int newSingerId = m_rotModel.singerAdd(ui->lineEditName->text(),
                                           ui->cbxPosition->currentIndex());
