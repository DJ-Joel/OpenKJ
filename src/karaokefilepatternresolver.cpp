...
{
    QSqlQuery query(
        SQL(SELECT sourceDirs.path,
            sourceDirs.pattern,
            custompatterns.name,
            custompatterns.artistregex,
            custompatterns.artistcapturegrp,
            custompatterns.titleregex,
            custompatterns.titlecapturegrp,
            custompatterns.discidregex,
            custompatterns.discidcapturegrp FROM sourceDirs LEFT JOIN custompatterns ON
                    sourceDirs.custompattern
                == custompatterns.patternid ORDER BY sourceDirs.path));

    while (query.next()) {
        auto pattern = static_cast<SourceDir::NamingPattern>(query.value(1).toInt());
        auto customPattern = pattern == SourceDir::CUSTOM && !query.isNull("name")
                                 ? CustomPattern(query.value("name").toString(),
        ...
        m_path_pattern_map[query.value(0).toString()]
            = KaraokeFilePatternResolver::KaraokeFilePattern{.pattern = pattern,
                                                             .customPattern = customPattern};
    }
    m_initialized = true;
}

const KaraokeFilePatternResolver::KaraokeFilePattern &KaraokeFilePatternResolver::getPattern(
...
static const KaraokeFilePatternResolver::KaraokeFilePattern defaultPattern{
    KaraokeFilePatternResolver::KaraokeFilePattern{.pattern = SourceDir::SAT}};
