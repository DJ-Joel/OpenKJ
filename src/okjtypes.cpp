...
{
    return os << " RotationSinger(id: " << s.id << " position: " << s.position
              << " name: " << s.name.toStdString() << " regular: " << s.regular
              << " addTS: " << s.addTs.toString().toStdString() << " valid: " << s.valid << ")";
}
...
QString RotationSinger::nextSongPath() const
{
    QSqlQuery query;
    query.prepare(
        "SELECT dbsongs.path FROM dbsongs,queuesongs WHERE queuesongs.singer = :singerid AND "
        "queuesongs.played = 0 AND dbsongs.songid = queuesongs.song ORDER BY position LIMIT 1");
    query.bindValue(":singerid", id);
    query.exec();
    if (auto lastError = query.lastError(); lastError.type() != QSqlError::NoError)
        m_logger->error("{} DB error! Error while querying the db on disk! Error: {}",
                        loggingPrefix(),
...
QString RotationSinger::nextSongArtist() const
{
    QSqlQuery query;
    query.prepare(
        "SELECT dbsongs.artist FROM dbsongs,queuesongs WHERE queuesongs.singer = :singerid AND "
        "queuesongs.played = 0 AND dbsongs.songid = queuesongs.song ORDER BY position LIMIT 1");
    query.bindValue(":singerid", id);
    query.exec();
    if (auto lastError = query.lastError(); lastError.type() != QSqlError::NoError)
...
QString RotationSinger::nextSongTitle() const
{
    QSqlQuery query;
    query.prepare(
        "SELECT dbsongs.title FROM dbsongs,queuesongs WHERE queuesongs.singer = :singerid AND "
        "queuesongs.played = 0 AND dbsongs.songid = queuesongs.song ORDER BY position LIMIT 1");
    query.bindValue(":singerid", id);
    query.exec();
    if (auto lastError = query.lastError(); lastError.type() != QSqlError::NoError)
...
{
    QSqlQuery query;
    query.prepare("SELECT dbsongs.artist, dbsongs.title FROM dbsongs,queuesongs WHERE "
                  "queuesongs.singer = :singerid AND queuesongs.played = 0 AND dbsongs.songid = "
                  "queuesongs.song ORDER BY position LIMIT 1");
    query.bindValue(":singerid", id);
    query.exec();
    if (auto lastError = query.lastError(); lastError.type() != QSqlError::NoError)
        m_logger->error("{} DB error! Error while querying the db on disk! Error: {}",
...
QString RotationSinger::nextSongSongId() const
{
    QSqlQuery query;
    query.prepare(
        "SELECT dbsongs.discid FROM dbsongs,queuesongs WHERE queuesongs.singer = :singerid AND "
        "queuesongs.played = 0 AND dbsongs.songid = queuesongs.song ORDER BY position LIMIT 1");
    query.bindValue(":singerid", id);
    query.exec();
    if (auto lastError = query.lastError(); lastError.type() != QSqlError::NoError)
...
int RotationSinger::nextSongDurationSecs() const
{
    QSqlQuery query;
    query.prepare(
        "SELECT dbsongs.duration FROM dbsongs,queuesongs WHERE queuesongs.singer = :singerid AND "
        "queuesongs.played = 0 AND dbsongs.songid = queuesongs.song ORDER BY position LIMIT 1");
    query.bindValue(":singerid", id);
    query.exec();
    if (auto lastError = query.lastError(); lastError.type() != QSqlError::NoError)
...
{
    QSqlQuery query;
    query.prepare("SELECT keychg FROM queuesongs WHERE singer = :singerid AND played = 0 ORDER BY "
                  "position LIMIT 1");
    query.bindValue(":singerid", id);
    query.exec();
    if (auto lastError = query.lastError(); lastError.type() != QSqlError::NoError)
...
{
    QSqlQuery query;
    query.prepare("SELECT qsongid FROM queuesongs WHERE singer = :singerid AND played = 0 ORDER BY "
                  "position LIMIT 1");
    query.bindValue(":singerid", id);
    query.exec();
    if (auto lastError = query.lastError(); lastError.type() != QSqlError::NoError)
...
int RotationSinger::numSongsSung() const
{
    QSqlQuery query;
    query.prepare(
        "SELECT COUNT(qsongid) FROM queuesongs WHERE singer = :singerid AND played = true");
    query.bindValue(":singerid", id);
    query.exec();
    if (auto lastError = query.lastError(); lastError.type() != QSqlError::NoError)
        m_logger->error("{} DB error! Error while querying the db on disk! Error: {}",
...
int RotationSinger::numSongsUnsung() const
{
    QSqlQuery query;
    query.prepare(
        "SELECT COUNT(qsongid) FROM queuesongs WHERE singer = :singerid AND played = false");
    query.bindValue(":singerid", id);
    query.exec();
    if (auto lastError = query.lastError(); lastError.type() != QSqlError::NoError)
        m_logger->error("{} DB error! Error while querying the db on disk! Error: {}",
...
RotationSinger::RotationSinger(
    int id, QString name, int position, bool regular, QDateTime addTs, bool valid)
    : id(id)
    , name(std::move(name))
    , position(position)
    , regular(regular)
    , addTs(std::move(addTs))
    , valid(valid)
{
    m_logger = spdlog::get("logger");
    m_settings = std::make_shared<Settings>();
