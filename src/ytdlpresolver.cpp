...
{
    if (isRunning()) {
        m_logger->warn("{} resolve() called while a resolve is already running, ignoring",
                       m_loggingPrefix);
        return;
    }
...
void YtDlpResolver::processFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (lines.size() > 1) {
        m_logger->warn("{} yt-dlp returned {} lines - using the first one only; playback may be "
                       "video-only (no audio)",
                       m_loggingPrefix,
                       lines.size());
    }
