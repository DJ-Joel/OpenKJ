...
SoftwareRenderVideoSink::SoftwareRenderVideoSink(QWidget *surface)
{
    ...
    m_videoCaps = gst_caps_new_simple("video/x-raw",
                                      ...
                                      G_TYPE_STRING,
                                      "RGB16",
                                      NULL); // QImage::Format_RGB16
    gst_caps_append(m_videoCaps,
                    gst_caps_new_simple("video/x-raw",
                                        "format",
                                        G_TYPE_STRING,
                                        "BGRx",
                                        NULL)); // QImage::Format_RGB32
...
void SoftwareRenderVideoSink::onSurfaceResized(const QSize &size)
{
    ...
    gst_caps_set_simple(m_videoCaps,
                        "width",
                        G_TYPE_INT,
                        size.width(),
                        "height",
...
GstFlowReturn SoftwareRenderVideoSink::NewSampleCallback([[maybe_unused]] GstAppSink *appsink,
                                                         gpointer user_data)
{
...
bool SoftwareRenderVideoSink::pullSampleAndDrawImage()
{
    ...
    if (sample) {
        ...
        gst_element_get_state(reinterpret_cast<GstElement *>(m_appSink),
                              &state,
                              nullptr,
                              GST_CLOCK_TIME_NONE);
